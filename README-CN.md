...
> 单变量和多变量在时间序列分析中通常用来描述模型所涉及的变量数量。
> 
> 单变量：指在分析中只涉及到一个变量的情况。例如，如果我们只考虑某个城市每天的温度变化，那么温度就是单变量数据。 
> 
> 多变量：指在分析中涉及到多个变量的情况。例如，如果我们不仅考虑某个城市每天的温度变化，还考虑到湿度、风速等变量，那么温度、湿度、风速等就是多变量数据。
> 
> 在LSTM模型中，单变量模型通常用于只考虑一个变量的情况，例如只使用温度的历史数据来预测未来的温度。而多变量模型则考虑到多个变量的历史数据，例如使用温度、湿度、风速等多个变量的历史数据来预测未来的温度、湿度、风速等多个变量的值。
